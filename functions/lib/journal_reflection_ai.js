...
exports.parseJournalReflectionJson = parseJournalReflectionJson;
exports.generateJournalReflection = generateJournalReflection;
const vertexai_1 = require("@google-cloud/vertexai");
function parseJournalReflectionJson(text) {
    ...
    try {
        ...
        const affirmation = typeof o.affirmation === "string" ? o.affirmation.trim() : "";
...
const JOURNAL_AI_CHARACTERS = {
    default: "You are a therapist-like assistant: warm, grounding, practical, and clear. " +
        "You do not insist on a personal name; if needed, refer to yourself neutrally (e.g. journal companion).",
    gyaru: "Your name is 美咲 (Misaki). You are a super positive gyaru-style AI: upbeat, friendly, fast-paced, and encouraging. " +
        "Stay playful but never crude or aggressive. You may sign or refer to yourself as 美咲 when natural.",
    kopitiam_uncle: "Your name is Wong. You are a kopitiam uncle persona: calm, warm, practical, and grounded. " +
        "Share short, useful life wisdom. You may refer to yourself as Wong when natural.",
    chinese_auntie: "Your name is Yin (阿姨 Yin). You are an energetic Chinese auntie persona: caring, direct, lively, and motivating. " +
        "Be strong but affectionate, never threatening or violent. You may refer to yourself as Yin / 阿姨 when natural.",
};
async function generateJournalReflection(project, content, category, journalPersonalization, importantProfile, recentHistory, characterId, userNickname) {
    ...
    const personalization = journalPersonalization.trim().slice(0, MAX_PERSONALIZATION_IN_PROMPT);
    const personalizationBlock = personalization.length > 0
    ...
    const importantProfileBlock = importantProfileJson.length > 0
    ...
    const recentForPrompt = recentHistory
        ...
        .join("\n");
    const recentHistoryBlock = recentForPrompt.length > 0
        ? `Recent conversation history (latest first):\n${recentForPrompt}\n\n`
        : "";
    const characterInstruction = JOURNAL_AI_CHARACTERS[characterId] ?? JOURNAL_AI_CHARACTERS.default;
    const characterBlock = `Character instruction:\n${characterInstruction}\n\n`;
    const nick = userNickname.trim().slice(0, 40);
    const nicknameBlock = nick.length > 0
        ? `The user asked to be addressed as “${nick}” when it fits naturally (do not overuse; skip if it breaks tone).\n\n`
        : "";
    const preferredLanguageRaw = importantProfile != null && typeof importantProfile["language"] === "string"
        ? importantProfile["language"].trim()
        : "";
    ...
    const prompt = personalizationBlock +
