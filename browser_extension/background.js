  ...
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    if (msg.action === "pomodoro_start") {
      ...
      chrome.storage.local.get(STORAGE_KEY, (data) => {
        ...
        if (state) {
          const remaining = Math.max(0, Math.floor((state.endTime - Date.now()) / 1e3));
          chrome.storage.local.set({
      ...
      chrome.alarms.get(POMODORO_ALARM).then((alarm) => {
        chrome.storage.local.get(STORAGE_KEY, (data) => {
          ...
          const remaining = running && state.endTime ? Math.max(0, Math.floor((state.endTime - Date.now()) / 1e3)) : state?.pausedSeconds ?? 0;
          sendResponse({
            running: !!alarm,
            remaining,
